  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-thin">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <motion.button whileTap={{ scale: 0.97 }}
              ...
              className={`flex-1 flex items-center gap-2 p-3 rounded-md text-xs font-medium transition-all duration-200 ${type === 'public' ? 'bg-[var(--accent)]/10 ring-1 ring-[var(--accent)]/30 text-[var(--accent)]' : 'bg-[var(--bg-base)] text-[var(--text-muted)]'}`}>
              <Hash className="h-4 w-4" /> Public
            </motion.button>
            <motion.button whileTap={{ scale: 0.97 }}
              onClick={() => canManage && setType('private')}
              className={`flex-1 flex items-center gap-2 p-3 rounded-md text-xs font-medium transition-all duration-200 ${type === 'private' ? 'bg-amber-500/10 ring-1 ring-amber-500/30 text-amber-400' : 'bg-[var(--bg-base)] text-[var(--text-muted)]'}`}>
        ...
        {/* Info */}
        <div className="rounded-md bg-[var(--bg-base)] shadow-card p-4 space-y-2.5 text-xs text-[var(--text-muted)]">
        ...
        {canManage && (
          <div className="rounded-md border border-red-500/20 bg-red-500/5 p-4 space-y-3">
            ...
            <motion.button whileTap={{ scale: 0.97 }}
              onClick={onArchive}
              className="w-full h-9 rounded-md border border-amber-500/20 bg-amber-500/5 text-xs text-amber-400 flex items-center justify-center gap-2 hover:bg-amber-500/10 transition">
              <Archive className="h-3.5 w-3.5" /> Archive Channel
            </motion.button>
            <motion.button whileTap={{ scale: 0.97 }}
              onClick={onDelete}
              className="w-full h-9 rounded-md border border-red-500/20 bg-red-500/5 text-xs text-red-400 flex items-center justify-center gap-2 hover:bg-red-500/10 transition">
