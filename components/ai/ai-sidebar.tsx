...
export default function AISidebar({ conversations, activeId, loading, onSelect, onNew, onDelete, onRename, onToggle }: Props) {
  ...
  return (
    <aside className="w-[260px] bg-[var(--bg-card)]/40 border-r border-[var(--border)] flex flex-col shrink-0">
      {/* Header */}
      <div className="p-3 border-b border-[var(--border)]">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..."
            className="w-full h-7 pl-7 pr-3 rounded-lg bg-[var(--bg-base)] border border-[var(--border)] text-[11px] text-[var(--text-secondary)] placeholder:text-[var(--text-muted)] outline-none focus:border-[#D4A843]/30 transition-colors" />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin" onClick={() => setMenuId(null)}>
        {loading ? (
          ...
          <div className="text-center py-10 px-4">
            ...
            <button onClick={onNew} className="text-[11px] text-[#D4A843] mt-1.5 hover:underline">Start a new chat</button>
          </div>
        ) : (
          <div className="p-1.5">
            {groups.map(group => (
                        ...
                        {isEditing ? (
                          <div className="flex items-center gap-1">
                            <input value={editTitle} onChange={e => setEditTitle(e.target.value)}
                              className="flex-1 h-6 px-1.5 rounded bg-[var(--bg-base)] border border-[#D4A843]/30 text-[11px] text-[var(--text-primary)] outline-none"
                      ...
                      <AnimatePresence>
                        {menuId === convo.id && (
                          <motion.div
                            ...
                            className="absolute right-1 top-9 z-20 w-32 py-1 rounded-xl bg-[var(--bg-card)] border border-[var(--border)] shadow-xl"
                            onClick={e => e.stopPropagation()}>
                            <button onClick={() => startRename(convo)} className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] text-[var(--text-secondary)] hover:bg-[var(--hover-bg)]">
