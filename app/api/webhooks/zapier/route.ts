...
export async function POST(req: NextRequest) {
  try {
    ...
    const zapId = req.headers.get('x-zapier-delivery-id') || '';
    if (!zapId) {
      return NextResponse.json({ error: 'x-zapier-delivery-id header is required' }, { status: 400 });
    }
    const isNew = await checkReplayProtection('zapier', zapId);
    if (!isNew) {
