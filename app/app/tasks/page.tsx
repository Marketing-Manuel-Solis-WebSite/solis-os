...
const VIS = [
  ...
  { id: 'public', label: 'Public', icon: Globe, color: '#22C55E' },
...
export default function TasksPage() {
  ...
  const load = useCallback(async () => {
    ...
    const visible = (t as any[]).filter(task => canSeeResource({
      teamId: task.teamId,
  ...
  const doDelete = async (t: any) => {
    if (!can('task', 'delete') && t.createdBy !== user?.uid) return alert('No permission to delete this task');
  ...
  return (
    <div className="flex h-[calc(100vh-64px)]">
              ...
              {groups.map(g => (
                <div key={g.key} className="w-72 shrink-0">
                  ...
                  <div className="space-y-2">
                    {g.tasks.map((t: any) => {
                      const p = PRI.find(x => x.id === t.priority);
                      const due = t.dueDate?.toDate?.();
                      const overdue = due && due < new Date() && t.status !== 'done';
                      const visConf = VIS.find(v => v.id === (t.visibility || 'team'));
                        ...
                        <div key={t.id} onClick={() => setSel(t)}
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {t.assignees?.slice(0, 2).map((uid: string) => {
                              ...
                              return <div key={uid} className="w-5 h-5 rounded-full bg-[#D4A843]/15 flex items-center justify-center text-[8px] font-bold text-[#D4A843]">{m?.displayName?.[0] || '?'}</div>;
                            })}
                          </div>
                        </div>
                      );
                ...
                <TaskGroup key={g.key} group={{ ...g, count: g.tasks.length }}>
                  <div className="space-y-1">
                    {g.tasks.map((t: any, i: number) => {
                      const st = STS.find(s => s.id === t.status) || STS[0];
                      const p = PRI.find(x => x.id === t.priority) || PRI[2];
                      const tp = TYP.find(x => x.id === (t.type || 'task')) || TYP[0];
                      const due = t.dueDate?.toDate?.();
                      const overdue = due && due < new Date() && t.status !== 'done';
      ...
      {/* Detail Panel */}
      {sel && <DetailPanel task={sel} members={members} teams={teams} uid={user!.uid} uname={me!.displayName} canUpdate={can('task', 'update')} canDelete={can('task', 'delete')} onUpdate={doUpdate} onDelete={doDelete} onClose={() => setSel(null)} />}

      {/* Create Modal */}
      {showCreate && <CreateModal members={members} teams={teams} activeTeamId={activeTeamId} onClose={() => setShowCreate(false)} onCreate={doCreate} />}
    </div>
  );
}

// === CREATE MODAL ===
function CreateModal({ members, teams, activeTeamId, onClose, onCreate }: any) {
...
// === DETAIL PANEL ===
function DetailPanel({ task, members, teams, uid, uname, canUpdate, canDelete, onUpdate, onDelete, onClose }: any) {
  ...
  const st = STS.find(s => s.id === task.status) || STS[0];
  const tp = TYP.find(t => t.id === (task.type || 'task')) || TYP[0];
  ...
  const visConf = VIS.find(v => v.id === (task.visibility || 'team'));
  const taskTeam = teams.find((t: any) => t.id === task.teamId);

  const postComment = async () => {
    if (!newCom.trim()) return;
    await addTaskComment(task.id, { text: newCom.trim(), authorId: uid, authorName: uname });
    setNewCom('');
    setComments(await getTaskComments(task.id));
  };
  const saveTitle = () => { if (titleVal.trim() && titleVal !== task.title && canUpdate) onUpdate(task.id, 'title', titleVal.trim(), task.title); setEditTitle(false); };
          ...
          <div className="p-5">
            {tab === 'comments' && (
              <div>
                ...
                <div className="flex gap-2">
                  <input value={newCom} onChange={e => setNewCom(e.target.value)} placeholder="Write a comment..." className="input-dark h-9 text-xs flex-1" onKeyDown={e => e.key === 'Enter' && postComment()} />
